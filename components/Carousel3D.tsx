...
import { translateGender, translatePitch, translateCharacteristic } from '../src/utils/translations';
            ...
            {voices.map((voice, index) => {
              ...
              return (
                <motion.div
                  ...
                  aria-label={`${voice.name} 목소리 카드. ${isActive ? 'Enter를 눌러 재생하세요.' : '클릭하여 선택하세요.'}`}
                >
                      ...
                      <div className="h-[40%] p-8 flex flex-col justify-between bg-white dark:bg-zinc-800 relative z-30">
                              ...
                              <div className="flex items-center justify-between mb-2">
                                  ...
                                  <div className="flex gap-2">
                                      <span className="px-2.5 py-1 rounded-full bg-zinc-100 dark:bg-zinc-700 text-zinc-600 dark:text-zinc-300 text-xs font-medium border border-zinc-200 dark:border-zinc-600">
                                          {translateGender(voice.analysis.gender)}
                                      </span>
                                  </div>
                              </div>
                              <div className="flex flex-wrap gap-2 mb-4">
                                  <span className="text-sm text-zinc-500 dark:text-zinc-400 font-light">{translatePitch(voice.pitch)} 톤</span>
                                  <span className="text-zinc-300 dark:text-zinc-600">•</span>
                                  <span className="text-sm text-zinc-500 dark:text-zinc-400 font-light">{translateCharacteristic(voice.analysis.characteristics[0])}</span>
                              </div>
                          </div>
                          ...
                          <div className="space-y-3">
                              ...
                              <p className="text-sm text-zinc-400 dark:text-zinc-500 font-light leading-relaxed line-clamp-2">
                                  {voice.analysis.characteristics.map(translateCharacteristic).join(', ')}
                              </p>
                          </div>
                      </div>
                  </div>
