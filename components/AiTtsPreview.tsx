...
const AiTtsPreview = forwardRef<AiTtsPreviewRef, AiTtsPreviewProps>(({
  text, maxChars, language, instruct, voiceName, onProcessingChange
}, ref) => {
    ...
    generate: async () => {
      ...
      try {
        ...
        for (let i = 0; i < textChunks.length; i++) {
          setProgress({ step: '오디오 생성 중...', current: i + 1, total: textChunks.length });
        ...
        alert("오디오 생성 중 오류가 발생했습니다. 콘솔을 확인해주세요.");
      } finally {
        setProcessing(false);
        setProgress(null);
      }
